...
type BankName = 'Bancolombia' | 'Nequi' | 'Banco de Bogota';
const BANKS: BankName[] = ['Bancolombia', 'Nequi', 'Banco de Bogota'];
const BANK_STORAGE_KEY = 'statistics.bank.initialBalances.v1';
...
export default function StatisticsPage() {
  ...
  const [initialBalances, setInitialBalances] = useState<Record<BankName, number>>({
    Bancolombia: 0,
    Nequi: 0,
    'Banco de Bogota': 0,
  });
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const parsed = JSON.parse(raw) as Partial<Record<BankName, number>>;
      setInitialBalances(prev => ({
        ...prev,
        ...Object.fromEntries(
          BANKS.map((b) => [b, typeof parsed[b] === 'number' && Number.isFinite(parsed[b] as number) ? (parsed[b] as number) : prev[b]])
        ),
      }));
    } catch {
      // ignore
  ...
  useEffect(() => {
    try {
      localStorage.setItem(BANK_STORAGE_KEY, JSON.stringify(initialBalances));
    } catch {
      // ignore
    }
  }, [initialBalances]);
  ...
  const handleSubmitExpense = async (e: React.FormEvent) => {
    ...
    try {
      await createExpense({
        ...
        date: formData.date,
        bank: formData.affectsBank ? (formData.bank || undefined) : undefined,
        createdBy: user.id,
      });
  ...
  const handleSubmitQuickRevenue = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;

    try {
      await createRevenue({
        serviceOrderId: `manual-${Date.now().toString()}`,
        amount: revenueFormData.amount,
        date: revenueFormData.date,
        description: revenueFormData.description,
        bank: revenueFormData.affectsBank ? (revenueFormData.bank || undefined) : undefined,
      });
      ...
      console.error('[v0] Error creating revenue:', error);
  ...
  const bankBalances: Record<BankName, number> = BANKS.reduce((acc, bank) => {
    const delta = bankMovements
      .filter(m => m.bank === bank)
      .reduce((sum, m) => sum + (m.type === 'in' ? m.amount : -m.amount), 0);
    acc[bank] = (initialBalances[bank] || 0) + delta;
    return acc;
  }, {} as Record<BankName, number>);
    ...
    <ProtectedRoute allowedRoles={['admin']}>
      <DashboardLayout title="Estadísticas y Reportes">
        <div className="space-y-6">
          ...
          {/* Registrar Gastos / Registrar Ingresos rápidos */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Registrar Gastos */}
            <Card>
              <CardHeader>
                ...
                <form onSubmit={handleSubmitExpense} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        id="expense-affects-bank"
                        type="checkbox"
                        checked={formData.affectsBank}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            affectsBank: e.target.checked,
                            bank: e.target.checked ? formData.bank : '',
                          })
                        }
                        className="h-4 w-4 rounded border-gray-300"
                      />
                      <Label htmlFor="expense-affects-bank" className="cursor-pointer">
                        ¿Este gasto sale de un banco?
                      </Label>
                    </div>
                    {formData.affectsBank && (
                      <div className="space-y-2">
                        <Label htmlFor="expense-bank">Banco</Label>
                        <Select
                          value={formData.bank}
            ...
            <Card>
              <CardHeader>
                <CardTitle>Registrar Ingresos (Rápido)</CardTitle>
                <CardDescription>Registra un ingreso manual sin crear una orden</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmitQuickRevenue} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="revenue-description">Descripción *</Label>
                    <Textarea
                      id="revenue-description"
                      value={revenueFormData.description}
                      onChange={(e) => setRevenueFormData({ ...revenueFormData, description: e.target.value })}
                      placeholder="Ej: Venta de repuesto, anticipo, servicio externo..."
                      rows={3}
                      required
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="revenue-amount">Monto *</Label>
                      <CurrencyInput
                        value={revenueFormData.amount}
                        onChange={(value) => setRevenueFormData({ ...revenueFormData, amount: value })}
                        placeholder="0"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="revenue-date">Fecha *</Label>
                      <Input
                        id="revenue-date"
                        type="date"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        id="revenue-affects-bank"
                        type="checkbox"
                        checked={revenueFormData.affectsBank}
                        onChange={(e) =>
                          setRevenueFormData({
                            ...revenueFormData,
                            affectsBank: e.target.checked,
                            bank: e.target.checked ? revenueFormData.bank : '',
                          })
                        }
                        className="h-4 w-4 rounded border-gray-300"
                      />
                      <Label htmlFor="revenue-affects-bank" className="cursor-pointer">
                        ¿Este ingreso entra a un banco?
                      </Label>
                    </div>
                    {revenueFormData.affectsBank && (
                      <div className="space-y-2">
                        <Label htmlFor="revenue-bank">Banco</Label>
                        <Select
                          value={revenueFormData.bank}
                          onValueChange={(value) => setRevenueFormData({ ...revenueFormData, bank: value as BankName })}
                        >
                          <SelectTrigger id="revenue-bank">
                            <SelectValue placeholder="Selecciona el banco" />
                          </SelectTrigger>
                          <SelectContent>
                            {BANKS.map((b) => (
                  ...
                  <Button
                    type="submit"
                    className="w-full"
                    disabled={!revenueFormData.description || revenueFormData.amount === 0}
                  >
                    <Plus className="h-4 w-4 mr-2" />
                    Registrar Ingreso
                  </Button>
          ...
          {/* Control de bancos (saldos y movimientos) */}
          <Card>
            <CardHeader>
              <CardTitle>Control de Bancos</CardTitle>
              <CardDescription>
                Saldos y movimientos calculados desde gastos/ingresos registrados con banco
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {BANKS.map((bank) => {
                  const movements = bankMovements.filter(m => m.bank === bank);
                  const balance = bankBalances[bank] ?? 0;
                  return (
                    <div key={bank} className="border rounded-lg p-4 space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Se guarda en este navegador (local).
                        </p>
                      </div>
                        ...
                        <div className="space-y-2 max-h-72 overflow-y-auto">
                          {movements.length === 0 ? (
                            <div className="text-sm text-muted-foreground">
                              Sin movimientos. Marca “¿sale/entra a un banco?” al registrar gastos o ingresos.
                            </div>
                          ) : (
                            movements.map((m) => (
                              <div
                                key={m.id}
                                className="flex items-center justify-between gap-3 p-2 border rounded-md hover:bg-muted/50"
                              >
                                <div className="flex-1 min-w-0">
                                  <div className="text-sm font-medium truncate">{m.description}</div>
                                  <div className="text-xs text-muted-foreground">
                                    {new Date(m.date || m.created_at).toLocaleDateString('es-ES', {
                                      year: 'numeric',
                                      month: 'short',
                                      day: 'numeric',
                                    })}
