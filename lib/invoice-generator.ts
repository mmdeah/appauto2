...
export function generateInvoiceHTML(
  ...
  // Calcular totales
  const subtotal = order.quotation.subtotal || order.quotation.items.reduce((sum, item) => sum + item.total, 0)
  const tax = order.quotation.tax || (order.quotation.includesTax ? subtotal * 0.19 : 0)
  const total = order.quotation.total || subtotal + tax
  ...
  if (!order.quotation.includesTax) {
