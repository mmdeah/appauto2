  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <div class="intro">
            ${customContent || `¡Hola <strong>${client.name}</strong>! Tu orden de servicio ha sido completada y está lista para ser entregada.`}
          </div>
          ...
          <div class="section">
            <div class="section-title">Servicios (${order.services.filter((s: any) => s.completed).length}/${order.services.length})</div>
            <div class="service-list">
              ${order.services.map((service: any) => `
                <div class="service-item ${service.completed ? 'service-completed' : 'service-pending'}">
          ...
          <div class="section">
            ...
            <div class="quality-item ${order.qualityControlCheck.vehicleClean ? 'quality-ok' : 'quality-fail'}">
              ${order.qualityControlCheck.vehicleClean ? '✓' : '✗'} Vehículo limpio
            </div>
            <div class="quality-item ${order.qualityControlCheck.noToolsInside ? 'quality-ok' : 'quality-fail'}">
              ${order.qualityControlCheck.noToolsInside ? '✓' : '✗'} Sin herramientas dentro
            </div>
            <div class="quality-item ${order.qualityControlCheck.properlyAssembled ? 'quality-ok' : 'quality-fail'}">
              ${order.qualityControlCheck.properlyAssembled ? '✓' : '✗'} Correctamente armado
            </div>
            <div class="quality-item ${order.qualityControlCheck.issueFixed ? 'quality-ok' : 'quality-fail'}">
              ...
              return `
                <div class="history-item">
                  ...
                  <div style="font-size: 12px; margin-top: 3px;">
                    ${item.notes || `Cambio a ${SERVICE_STATE_LABELS[item.newState] || item.newState}`}
                  </div>
